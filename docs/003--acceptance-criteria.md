...
- `apps/daemon/tests/proto_ssh_cold_start.rs::cold_start_walks_every_stage_through_real_sshd_and_handshake_completes` — T4 SSH-transport-tier coverage: drives `prepare_remote` + `attach_agent` in-process against the `common::ssh` real-sshd harness, asserts all seven stage callbacks fire in order (`VersionProbe` → `TarballStage` → `Scp` → `RemoteBuild` → `DaemonSpawn` → `SocketTunnel` → `SocketConnect`), the handshake completes, and `agent.version` lands on disk with `bootstrap_version()`'s value. Hermeticity: HOME is overridden to a per-test tempdir via `spawn_sshd`'s env-injection; `cargo` is shimmed on the SSH PATH so `RemoteBuild` is a no-op that copies the pre-built `CARGO_BIN_EXE_codemuxd` into place; `claude` is symlinked on the same PATH so the daemon spawns the in-tree `fake_daemon_agent` without any production-code change.
- `apps/daemon/tests/proto_ssh_cold_start.rs::warm_start_skips_install_stages_when_remote_version_marker_matches` — pins the warm-start assertion end-to-end against real sshd: a second `prepare_remote` against the same scratch HOME fires only `VersionProbe` (not `TarballStage`/`Scp`/`RemoteBuild`), and the subsequent `attach_agent` still walks `DaemonSpawn` → `SocketTunnel` → `SocketConnect` and completes the handshake.
...
- `apps/tui/tests/pty_spawn_autocomplete.rs::tab_in_path_zone_renders_wildmenu_and_applies_selection` — boots codemux with `[spawn] default_mode = "precise"` and a tempdir seeded with three subdirectories (`aaa/`, `aab/`, `bbb/`), opens the modal, clears the auto-seeded path with `Ctrl+U`, types `<tempdir>/a`, waits for both `aaa` and `aab` candidates to render in the wildmenu, then presses `Tab` and asserts (a) the path zone now shows `aaa`, (b) the wildmenu transitioned to the no-matches sentinel, and (c) the unpicked sibling `aab` is no longer visible. Pins the chord-to-typed-prefix-to-Tab-applies pipeline end-to-end through the real keymap. The basename-prefix filter coverage (`bbb` excluded) is left to the `scan_dir_filters_out_files` unit test so the E2E test pins one concern, not two.
...
- `apps/tui/tests/pty_spawn_alias.rs::saved_alias_in_config_resolves_to_real_path` — boots codemux with a `[[spawn.projects]]` entry pointing at a tempdir, flips to `LeftPane` chrome, opens the modal in default Fuzzy mode, types the alias name, asserts the wildmenu surfaces a row containing both the alias AND the project tempdir's basename (proving the saved project flowed through `score_fuzzy` to the rendered wildmenu), then presses Enter and asserts a second agent (`[2]`) appears in the navigator. Local-only project (no `host = ...`) so the spawn routes through `ModalOutcome::Spawn` and stays hermetic; the bound-host `PrepareHostThenSpawn` path is left uncovered (needs a real `sshd` to exercise end-to-end).
...
**Tests:**
- `apps/daemon/tests/proto_ssh_disconnect.rs::daemon_survives_ssh_controlmaster_kill` — slow-tier real-sshd test covering the SSH-transport half. Spawns a real `sshd` subprocess on a `127.0.0.1`-bound random port via the `common::ssh` harness, opens an SSH `ControlMaster`, launches `codemuxd` over the master under `setsid -f` (the production-bootstrap shape, including the `</dev/null >...stderr 2>&1` redirect), kills the master with `ssh -O exit`, then asserts the daemon's pid is still alive via `kill -0` plus a `/proc/<pid>/stat` zombie-state check. Final belt-and-suspenders: a fresh ssh session re-runs `kill -0` on the host. Linux-only.
- `apps/daemon/tests/proto_setsid.rs::daemon_spawned_under_setsid_becomes_its_own_session_leader` — T4 wire-tier coverage of the daemon-side process-group invariant: spawns `codemuxd` under `setsid` (mirroring the bootstrap), reads `/proc/<pid>/stat` field 6, asserts `sid == pid` (the daemon is a session leader) and `daemon_sid != test_sid` (it actually detached from the spawning session). A future refactor that re-acquires a controlling terminal (e.g. opening `/dev/tty` in `init_tracing`) would break this assertion. Linux-only (`#[cfg(target_os = "linux")]`); on non-Linux hosts the AC is verified manually via the bootstrap's redirect test plus production behaviour.
