...
- (partially uncovered: real end-to-end SSH cold-start that walks every stage indicator via a real sshd. The `apps/daemon/tests/common/ssh.rs` harness (shipped for AC-043) now provides a real-sshd subprocess on `127.0.0.1`, but a hermetic cold-start test still needs (a) a daemon-side env-var hook so the spawned `codemuxd` runs an in-tree fake instead of `claude` — the production daemon CLI hard-codes `claude` as the default and the bootstrap shells out without a trailing argv, and (b) acceptance that a fresh-`$HOME` cargo build of `codemuxd` over ssh is acceptable for CI (slow, network-dependent for crates registry, writes into the user's real `~/.cache/codemuxd/`). Verified by hand against production SSH endpoints until those two are resolved.)
