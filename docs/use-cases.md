...
codemux doesn't try to be a code-review tool. It gives me a one-keystroke bridge between the two depths. Flagging is always "type it back to Claude" — never through a codemux-specific UI.
...
- **Structured code review** — annotations, "send this hunk back as a comment," approval flows. Use GitHub, an IDE, or `$EDITOR`. codemux's edits panel is read-only.
...
- **Cross-tool orchestration** — running Codex, Cursor Agent, or Gemini alongside Claude Code. codemux is Claude Code only.
...
- **Session archival / export / handoff** — markdown transcripts, paste-into-Slack formatting, re-attachable shareable links. Not in scope.
